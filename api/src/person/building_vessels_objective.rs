...
pub(crate) enum BuildingVesselsObjective {
    ...
    pub(crate) fn pursue(
        ...
        match self {
            BuildingVesselsObjective::SearchingForDockyard {
                ...
                Err(BuildingVesselsObjectiveError::CanNotFindDockyard)
            }
            BuildingVesselsObjective::MovingToDockyardModule {
                dst,
                needed_capabilities,
            } => {
            ...
            BuildingVesselsObjective::Building {
                needed_capabilities,
                process_token,
            } => match process_token {
                None => {
                    if let Some(modules) = Self::find_modules_with_capabilities(
                        ...
                        *process_token = Some(
                        ...
                        Err(BuildingVesselsObjectiveError::CanNotFindExpectedModulesUponArrivalInDockyard)
                    }
                }
                Some(process_token) => {
                    if process_token
            ...
            BuildingVesselsObjective::Done => todo!(),
        }
    }
...
#[derive(Debug)]
pub(crate) enum BuildingVesselsObjectiveError {
    CanNotFindDockyard,
    CanNotFindExpectedModulesUponArrivalInDockyard,
}

impl Display for BuildingVesselsObjectiveError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        todo!()
    }
}

impl Error for BuildingVesselsObjectiveError {}
